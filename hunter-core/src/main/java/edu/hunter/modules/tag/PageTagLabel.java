 ...
 * 
 */
public class PageTagLabel extends TagSupport {
	...
	public int doEndTag() throws JspException {
		...
		outValue.append(align).append("' style='").append(style).append("' >");
		try {
			...
			outValue.append("alert('需要跳转的页面超出页面范围 1 至 ").append(int_totalPages)
					.append(" 页！');");
			outValue.append("document.getElementById('toPageIndex').value = '';");
			outValue.append("return;").append("}}");
			outValue.append("if('").append(int_totalPages).append("' < 2 || '")
					.append(int_currentPage).append("' == opval){");
			...
			if ("true".equalsIgnoreCase(isDialogLocaltion)) {
				...
				if (pos > 0) {
					outValue.append(urlAction).append("&pageIndex='+opval;");
				} else {
					outValue.append(urlAction).append("?pageIndex='+opval;");
				}
